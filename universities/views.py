...
def universities_page(request):
    ...
    if search_query:
        data = University.objects.filter(
            Q(name__icontains=search_query) | Q(location__icontains=search_query)).distinct().order_by('id')
        title = f'Search results for {search_query}'
    else:
        data = University.objects.all().order_by('id')
...
def university_detail(request, slug):
    ...
    prev_uni = University.objects.filter(
        id__lt=university.id).order_by('rank_order').last()
    next_uni = University.objects.filter(
